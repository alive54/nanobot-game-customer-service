...
def _pick_routable_target(
    session_manager,
    enabled_channels: Iterable[str],
    *,
    allow_cli_fallback: bool,
) -> tuple[str, str] | None:
    """Pick a routable channel/chat target from known sessions."""
...
def _build_gateway_http_app(bus, session_manager, enabled_channels: list[str]):
    ...
    app = FastAPI(title="Nanobot Gateway", version=__version__)
    ...
    @app.post("/message", tags=["gateway"])
    async def post_message(payload: GatewayMessageIn) -> dict[str, Any]:
        if payload.channel and payload.chat_id:
            if payload.channel not in enabled_channels:
                raise HTTPException(status_code=400, detail="channel not enabled")
            ...
            if target is None:
                raise HTTPException(
                    status_code=409,
                    detail="no routable external session; provide channel and chat_id explicitly",
                )
...
def gateway(
    ...
    gateway_app = _build_gateway_http_app(bus, session_manager, channels.enabled_channels)
    ...
    console.print(
        f"[green]鉁揫/green] HTTP message endpoint: POST http://{gateway_host}:{port}/message"
    )
    ...
    async def run():
        try:
            ...
            http_task = asyncio.create_task(gateway_server.serve())
