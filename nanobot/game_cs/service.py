...
_TMPL: dict[str, dict[str, str]] = {
    ...
    SOPState.GREETING: {
        ...
        "professional": (
            "您好，欢迎咨询顽石英雄客服。为准确为您服务，请提供：区服（几区）以及您的角色名。"
        ),
        "steady": (
            "老板您好，小妹在这儿呢。麻烦告知一下您是在哪个区，以及角色名，小妹来帮您安排兑换码。"
        ),
        "humorous": ("哥，掉线了还是来找小妹补给了😄 快告诉小妹几区+角色名，兑换码已经蓄势待发！"),
    },
    ...
    SOPState.FOLLOW_UP_30MIN: {
        ...
        "humorous": ("哥，小妹来给你送buff了！🎲 签到3天能抽大奖，最高充值赞助，要不要上车？"),
    },
    ...
    SOPState.FOLLOW_UP_1HOUR: {
        ...
        "humorous": (
            "哥，帮你锁定席位了！🎰 拉朋友来玩，最高1000真冲等你拿，人多爆率也高哦，快叫队友！"
        ),
    },
    # ── NEXT_DAY_VISIT ────────────────────────────────────────────────────────
    SOPState.NEXT_DAY_VISIT: {
        "lively": (
            "老板下午好 打扰一下下 就是想问问您，昨天的那款传奇 还在玩嘛 可以找我领取每日福利的😊"
        ),
        "professional": (
            "您好，请问您昨天的游戏体验如何？如有需要，今日福利可继续领取，欢迎随时联系。"
        ),
        "steady": ("老板，昨天玩得咋样？有没有继续上线呢？每天找小妹签到可以领福利哦。"),
        "humorous": ("哥，昨天爆出啥好装备了没？😄 每日签到福利还等着你，快来！"),
    },
    # ── REACTIVATION ──────────────────────────────────────────────────────────
    SOPState.REACTIVATION: {
        ...
        "professional": (
            "您好，近期游戏新区开放，装备爆率大幅提升。欢迎回游体验，如需协助请联系客服。"
        ),
        "steady": ("老板，好久不见！新区刚开，爆率翻倍，要不要回来看看，小妹帮您准备了回归礼。"),
        "humorous": ("哥，消失这么久，是练到满级了还是去充钱了？😏 新区爆率翻倍，快回来薅羊毛！"),
    },
    # ── EXCEPTION ────────────────────────────────────────────────────────────
    SOPState.EXCEPTION: {
...
def _handle_message(
    ...
    *,
    ...
    def _reply(
        text: str, *, next_step: str | None = None, bound: bool = False, codes: dict | None = None
    ) -> GameReply:
    ...
    if session.sop_state == SOPState.VALIDATING:
        ...
        if validation_ok:
            ...
            msg = "哥，没查到这个角色呢，是不是区服或者名字输错啦？麻烦再发一下几区和角色名~"
    ...
    if session.sop_state in (
        ...
        if user_text:
            ...
            return _kb_reply(
                user_id, user_text, cfg, store, kb, session, personality, ai_runtime=ai_runtime
            )
...
def _kb_reply(
    ...
    *,
    ...
    if cfg.ai_enabled and ai_runtime is not None:
        ...
        try:
            ...
            if len(history_for_ai) >= 2:
                _kb_ctx = kb.search_with_context(
                    user_text,
                ...
                _kb_ctx = kb.search(user_text, limit=10, include_l2=True)
            _log.info(
                "KB Serch for user %s in state %s returned %s results",
                user_id,
                session.sop_state,
                _kb_ctx,
            )
            ...
            _logging2.getLogger(__name__).warning(
                "[AI] _kb_reply AI path raised: %s — falling back to KB",
                _exc,
                exc_info=False,
            )
    ...
    if len(history) >= 2:
        kb_lines = kb.search_with_context(
            user_text,
            history=history,
            limit=4,
            include_l2=True,
        ...
        kb_lines = kb.search(user_text, limit=4, include_l2=True)
    print(f"KB Search for user {user_id} in state {session.sop_state} returned {kb_lines}")
    # Low-confidence guard: if top KB score is below threshold, hand off to human.
    scores: list[float] = []
...
def create_app(config: GameCSConfig | None = None) -> FastAPI:
    ...
    if cfg.ai_enabled:
        try:
            ...
            _ai_runtime = build_runtime(
                workspace_path=r"C:\Users\Administrator\.nanobot\workspace1",
                timeout_ms=cfg.ai_timeout_ms,
            )
            ...
            logging.getLogger(__name__).warning(
                "Failed to initialise AI runtime: %s. Running without AI.",
                _exc,
                exc_info=False,
            )
