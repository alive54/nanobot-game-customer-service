...
class GameCSConfig:
    ...
    def from_env() -> "GameCSConfig":
        ...
        data_dir = Path(os.getenv("GAME_CS_DATA_DIR", ".nanobot"))

        return GameCSConfig(
            service_token=os.getenv("GAME_CS_SERVICE_TOKEN", "dev-token"),
            db_driver=os.getenv("GAME_CS_DB_DRIVER", "mysql").strip().lower(),
            db_path=Path(os.getenv("GAME_CS_DB_PATH", str(data_dir / "game_cs.db"))),
            db_host=os.getenv("GAME_CS_DB_HOST", "localhost").strip(),
            db_port=int(os.getenv("GAME_CS_DB_PORT", "3306")),
            db_user=os.getenv("GAME_CS_DB_USER", "root").strip(),
            db_password=os.getenv("GAME_CS_DB_PASSWORD", "root"),
            db_name=os.getenv("GAME_CS_DB_NAME", "nanobot_game_cs").strip(),
            data_dir=data_dir,
            uploads_dir=Path(os.getenv("GAME_CS_UPLOADS_DIR", str(data_dir / "game_cs_uploads"))),
            openviking_path=Path(os.getenv("GAME_CS_OPENVIKING_PATH", str(data_dir / "openviking_data"))),
            openviking_target_uri=os.getenv("GAME_CS_OPENVIKING_TARGET_URI", "viking://resources/game-cs/"),
            ...
            kb_handoff_score_threshold=float(os.getenv("GAME_CS_KB_HANDOFF_SCORE_THRESHOLD", "0.45")),
            ai_tool_whitelist=tuple(
                s.strip() for s in os.getenv("GAME_CS_AI_TOOL_WHITELIST", "").split(",") if s.strip()
            ),
            ai_info_extract_confidence_threshold=float(
            ...
            intent_api_base=os.getenv("GAME_CS_INTENT_API_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1"),
            ...
            admin_gateway_url=os.getenv("GAME_CS_ADMIN_GATEWAY_URL", "http://127.0.0.1:18790/message"),
            ...
            vision_enabled=_env_bool("GAME_CS_VISION_ENABLED", True),
            vision_api_key=dashscope_api_key,
            vision_api_base=os.getenv("GAME_CS_VISION_API_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1"),
